...
const GOOGLE_CLIENT_ID = process.env.REACT_APP_GOOGLE_CLIENT_ID || '83745494475-om5dg3d440dhnh500ncrbpbkar7ev4s5.apps.googleusercontent.com';
...
const AdminLoginContent: React.FC<AdminLoginProps> = ({ onLogin }) => {
  ...
  const googleClientIdConfigured = GOOGLE_CLIENT_ID && GOOGLE_CLIENT_ID.length > 0;
    ...
    onSuccess: async (codeResponse) => {
      ...
      try {
        ...
        if (googleResponse.data.user?.role !== 'admin') {
          setError('Admin access only. Please use the Admin Login page for admin accounts.');
  ...
  const handleGoogleLoginClick = () => {
    if (!googleClientIdConfigured) {
      setError('Google login is not configured. Please use email and password to login.');
      return;
    }
    ...
    <div className="auth-container figma-login-bg">
      <div className="auth-card figma-login-card">
        ...
        <form onSubmit={handleSubmit} className="auth-form">
          ...
          <button type="submit" className="btn btn-primary figma-login-btn" disabled={loading || googleLoading}>
